...
async function availablePort(preferred = 3010) {
  ...
  throw new Error('No Vite port is available for the Freelance Briefs smoke test.');
}
...
const project = {
  id: 'freelance-briefs-real-inference', projectName: 'Space Marines Freelance Briefs', lastModified: Date.now(), workflowState: 3, projectType: 'GAME',
  ...
  mvpFeatureSpecValidation: fixture.mvpFeatureSpecValidation || [], tddContent: fixture.tddContent, technicalDesignDocument: fixture.technicalDesignDocument,
  assetList: fixture.assetList, assetMetadata: fixture.assetMetadata || null, productionBriefs: null,
  scopeReviewContent: fixture.scopeReviewContent, scopeReviewLens: null, modularBreakdown: null,
...
try {
  ...
  if (!Boolean((await button.locator('h4').getAttribute('class').catch(() => ''))?.includes('text-green-400'))) throw new Error('Freelance Briefs did not complete before its isolated timeout.');
  ...
  for (let attempt = 0; attempt < 20; attempt += 1) {
    ...
    if (saved) {
      ...
      if (generated?.productionBriefs?.length && generated?.modularBreakdown?.length) break;
    }
    await sleep(500);
  }
  if (!generated?.productionBriefs?.length || !generated?.modularBreakdown?.length) throw new Error('The generated production briefs were not persisted.');
  if (generated.productionBriefs.some(brief => !brief.id?.trim() || !brief.role?.trim() || !brief.deliverables?.length || !brief.acceptanceCriteria?.length)) {
    throw new Error('The persisted production briefs contain an invalid required field.');
  }
  console.log(`Isolated Freelance Briefs smoke passed with ${generated.productionBriefs.length} structured briefs.`);
} finally {
  await browser?.close();
  server.kill('SIGTERM');
  proxy.kill('SIGTERM');
